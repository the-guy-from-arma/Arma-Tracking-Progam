...
export function AuthForm({
  ...
  return (
    <main className={`access ${university ? "universityAccess" : owner ? "ownerAccessPage" : ""}`}>
      ...
      <section className="accessIntro">
        ...
        <p className="kicker">{owner ? "RESTRICTED INSTITUTIONAL CONTROL" : university ? "ONLINE CAMPUS + STUDENT INFORMATION SYSTEM" : "ARMA DEVELOPMENT + STUDIO NETWORK"}</p>
        <h1>{owner ? <>Institutional control.<br /><em>Owner authority only.</em></> : university ? <>Study with purpose.<br /><em>Build with evidence.</em></> : <>Build your craft.<br /><em>Prove it in the field.</em></>}</h1>
        <p>{owner ? "A separate administrative surface for admissions, sponsorship, curriculum synchronization, AI assessment operations, and audit controls." : university ? "Enfusion University is a complete online campus for structured courses, assessed mod assignments, academic pathways, and durable learner records." : "Project VALORIS brings live development work, studio collaboration, approvals, and community leadership into one professional network."}</p>
      ...
      <form className="accessCard" onSubmit={submit}>
        <header>
          ...
          <div>
            <h2>{owner ? "Owner administration sign in" : mode === "login" ? university ? "Student sign in" : "Enter Project VALORIS" : university ? "Apply to Enfusion University" : "Join Project VALORIS"}</h2>
            <p>{owner ? "Use the Railway-configured owner account. Student accounts are refused here." : mode === "login" ? university ? "Use your internal EFU ID or recovery email." : "Open your development workspace." : university ? "Your internal institution identity is issued after registration." : "New members begin as developing creators."}</p>
          </div>
        </header>
        {mode === "register" && <>
        ...
        <button className="primary" disabled={busy}>{busy ? "ESTABLISHING LINK…" : owner ? "ENTER OWNER ADMINISTRATION →" : mode === "login" ? university ? "ENTER STUDENT PORTAL →" : "ENTER VALORIS →" : university ? "CREATE STUDENT ACCOUNT →" : "CREATE MEMBER PROFILE →"}</button>
        {!owner && <p className="formSwitch">
...
function Brand({ university, owner }: { university: boolean; owner: boolean }) {
  return <div className="brand"><b>{owner ? "OA" : university ? "EU" : "V"}</b><span><strong>{owner ? "OWNER ADMINISTRATION" : university ? "ENFUSION UNIVERSITY" : "PROJECT VALORIS"}</strong><small>{owner ? "RESTRICTED · AUDITED · SEPARATE" : university ? "LEARN · BUILD · DEMONSTRATE" : "BUILD · COLLABORATE · SHIP"}</small></span></div>;
}
