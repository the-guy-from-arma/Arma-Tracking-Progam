...
async function reconcileUser(user) {
  ...
  await db.$transaction(async (tx) => {
    for (const source of candidates) {
      const award = await tx.fundingAward.create({ data: { referenceNumber: `BACKFILL-${source.entry.id}`, userId: user.id, type: typeFor(source.entry.type), status: source.remaining <= 0 ? "FULLY_USED" : source.remaining < source.original ? "PARTIALLY_USED" : "AVAILABLE", sourceName: source.entry.description, originalAmountCents: source.original, remainingAmountCents: source.remaining, awardedAt: source.entry.createdAt, publicDescription: source.entry.description, restrictions: "Eligible Enscript University learning services only; noncashable and nontransferable.", issuingDepartment: "University Sponsorship Office", legacy: true } });
