...
import { queueFacultyReply, requestFacultySupport, retryFacultyReply, studentFacultyConversations } from "@/lib/faculty-network";
...
export async function POST(request: Request) {
  ...
  if (action === "send") {
    try { return NextResponse.json(await queueFacultyReply(user.id, conversationId, text(body.message, 2400), text(body.clientMessageId, 100)), { status: 202 }); }
    catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Message could not be sent" }, { status: 400 }); }
  }
  if (action === "retry") {
    try { return NextResponse.json({ job: await retryFacultyReply(user.id, conversationId, text(body.jobId, 100)) }, { status: 202 }); }
    catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Response could not be retried" }, { status: 400 }); }
  }
  if (action === "support") {
    try { return NextResponse.json({ job: await requestFacultySupport(user.id, conversationId, text(body.jobId, 100)) }, { status: 202 }); }
    catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Support could not be requested" }, { status: 400 }); }
  }
