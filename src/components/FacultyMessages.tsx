...
type Message = { id: string; senderRole: string; body: string; createdAt: string; optimistic?: boolean };
type Conversation = {
  ...
  course: { code: string; title: string } | null; messages: Message[]; replyJobs: { id: string; status: string; attempt: number; maxAttempts: number; availableAt: string; lastError: string | null; supportRequestedAt: string | null }[];
};
...
export function FacultyMessages({ initialConversationId }: { initialConversationId?: string } = {}) {
  ...
  const load = useCallback(async () => {
    const response = await fetch("/api/university/messages", { cache: "no-store", signal: AbortSignal.timeout(15000) });
  ...
  useEffect(() => {
    ...
    const processing = data.conversations.some((item) => item.replyJobs.some((job) => job.status === "PROCESSING"));
    const timer = setInterval(() => void load().catch(() => setNotice("Campus Messages is reconnecting. Your conversation is preserved.")), processing ? 2000 : 5000);
  ...
  async function send(event: FormEvent) {
    ...
    const body = message.trim(); const clientMessageId = crypto.randomUUID().replaceAll("-", "");
    setBusy(true); setNotice("Message received by Campus Messages. Your faculty response is being prepared."); setMessage("");
    setData((current) => current ? { ...current, conversations: current.conversations.map((conversation) => conversation.id === selected.id ? { ...conversation, messages: [...conversation.messages, { id: clientMessageId, senderRole: "STUDENT", body, createdAt: new Date().toISOString(), optimistic: true }], replyJobs: [...conversation.replyJobs, { id: `pending-${clientMessageId}`, status: "QUEUED", attempt: 0, maxAttempts: 3, availableAt: new Date().toISOString(), lastError: null, supportRequestedAt: null }] } : conversation) } : current);
    try {
      const response = await fetch("/api/university/messages", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "send", conversationId: selected.id, message: body, clientMessageId }), signal: AbortSignal.timeout(15000) });
      const result = await response.json(); if (!response.ok) throw new Error(result.error || "Message could not be sent"); await load();
    } catch (error) { setNotice(error instanceof Error ? error.message : "Message could not be sent. Retry when your connection returns."); await load().catch(() => undefined); }
    finally { setBusy(false); }
  }

  async function jobAction(action: "retry" | "support", jobId: string) {
    if (!selected) return; setNotice(action === "retry" ? "The response was returned to the faculty queue." : "A support request was opened.");
    try { const response = await fetch("/api/university/messages", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, conversationId: selected.id, jobId }), signal: AbortSignal.timeout(15000) }); const result = await response.json(); if (!response.ok) throw new Error(result.error); await load(); }
  ...
  return <section className={styles.messages}>
    ...
    <div className={styles.workspace}>
      ...
      <AnimatePresence mode="wait">{selected && <motion.article key={selected.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
        ...
        <div className={styles.thread}>{selected.messages.map((item) => <div className={item.senderRole === "STUDENT" ? styles.student : styles.faculty} key={item.id}><small>{item.senderRole === "STUDENT" ? "YOU" : item.senderRole === "SYSTEM" ? "CAMPUS SUPPORT" : selected.facultyProfile.name}</small><p>{item.body}</p><time>{item.optimistic ? "Sending securely…" : new Date(item.createdAt).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}</time></div>)}{selected.replyJobs.filter((job) => ["QUEUED", "PROCESSING", "WAITING_FOR_CONSENT", "EXCEPTION"].includes(job.status)).map((job) => <div className={styles.preparing} key={job.id}><i /><span>{job.status === "QUEUED" ? "Response queued" : job.status === "PROCESSING" ? `${selected.facultyProfile.name} is preparing a reply` : job.status === "WAITING_FOR_CONSENT" ? "Waiting for your current policy acceptance" : "Response needs support"}{job.status === "EXCEPTION" && <><button onClick={() => void jobAction("retry", job.id)}>Retry response</button><button onClick={() => void jobAction("support", job.id)} disabled={Boolean(job.supportRequestedAt)}>{job.supportRequestedAt ? "Support requested" : "Request support"}</button></>}</span></div>)}</div>
