...
import { ArrowLeft, ArrowRight, BookOpen, Check, ChevronDown, CircleAlert, Clock3, ExternalLink, FileText, FlaskConical, Focus, GraduationCap, Maximize2, Menu, Moon, Printer, Save, Sun, Wrench, X } from "lucide-react";
...
function CourseOverview({ course, refresh }: { course: CourseStudio; refresh: () => Promise<void> }) {
  ...
  return <motion.div className={styles.overview} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
    ...
    <section className={styles.syllabusSection} id="syllabus"><header><div><small>DAY-BY-DAY CURRICULUM</small><h2>Your guided studio sequence</h2></div><p>Each day combines source-grounded instruction, exact Workbench actions, verification, troubleshooting, practical work, and a knowledge check.</p></header><ol className={styles.syllabus}>{course.days.map((day) => <li key={day.id} data-complete={Boolean(day.progress?.completed)}><span>{day.progress?.completed ? <Check /> : String(day.dayNumber).padStart(2, "0")}</span><div><small>DAY {day.dayNumber} · {day.estimatedMinutes} MINUTES</small><h3>{day.title}</h3><p>{day.objectives[0]}</p></div>{course.enrollment ? <Link href={`/university/courses/${course.id}/lessons/${day.dayNumber}`}>{day.progress?.completed ? "Review" : "Open studio"} <ArrowRight size={16} /></Link> : <button onClick={() => setConfirm(true)}>Preview enrollment</button>}</li>)}</ol></section>
...
function LessonReader({ course, dayNumber, refresh }: { course: CourseStudio; dayNumber: number; refresh: () => Promise<void> }) {
  ...
  async function save(action = "SAVE_DRAFT") {
    ...
    const response = await fetch(`/api/curriculum/${course.id}/lessons/${activeLesson.dayNumber}/progress`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, stepState: steps, developmentNotes: notes, reflection, answerDraft: { answer }, answer, labConfirmed: lab, readingPosition: Math.round((scrollY / Math.max(1, document.documentElement.scrollHeight - innerHeight)) * 100) }) });
    const result = await response.json().catch(() => ({})); setSaving(false);
    setMessage(response.ok ? action === "COMPLETE" ? `Day complete · knowledge score ${result.score}%` : "Studio notes and checkpoints saved." : result.error || "Your work could not be saved.");
  ...
  return <div className={styles.readerShell}>
    ...
    <motion.article className={styles.lessonReader} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <header><small>DAY {lesson.dayNumber} OF {course.days.length} · {course.academy}</small><h1>{lesson.title}</h1><div><span><Clock3 size={16} /> {lesson.estimatedMinutes} minutes</span><span><GraduationCap size={16} /> Guided studio</span><span><FileText size={16} /> Version {lesson.version?.number || "legacy"}</span></div></header>
      <section className={styles.objectivePanel}><small>WHEN YOU FINISH</small><h2>You will be able to</h2><ul>{lesson.objectives.map((item) => <li key={item}><Check size={17} />{item}</li>)}</ul></section>
      {lesson.blocks.map((block, index) => <ContentBlock key={block.id} block={block} number={index + 1} steps={steps} setSteps={setSteps} media={media} openMedia={setMediaOpen} sources={lesson.sources} />)}
