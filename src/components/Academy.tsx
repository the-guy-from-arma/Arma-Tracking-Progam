...
export function Academy({ initialTab = "catalog", context = "valoris" }: { initialTab?: "catalog" | "programs" | "submissions" | "credentials" | "review"; context?: "valoris" | "university" }) {
  ...
  return <div className="academy">
    ...
    <div className="academyNotice"><b>COMMUNITY EDUCATION NOTICE</b><span>{university ? "Enfusion University programs are community-based and non-accredited unless a recognized authority formally states otherwise." : "Project VALORIS certificates and degree-style pathways are non-accredited unless a recognized institution separately approves or accepts them."}</span></div>
...
function SubmissionRow({ submission }: { submission: Submission }) {
  return <article className="submissionRow"><span className={`submissionStatus ${submission.status.toLowerCase()}`}>{submission.status.replaceAll("_", " ")}</span><div><strong>{submission.title}</strong><small>{submission.course.code} · {submission.course.title} · {submission.student.name}</small>{submission.feedback && <p>Reviewer note: {submission.feedback}</p>}</div><a href={submission.repositoryUrl} target="_blank" rel="noreferrer">SOURCE ↗</a>{submission.certificate && <Link href={`/credentials/${submission.certificate.credentialCode}`}>CREDENTIAL ↗</Link>}</article>;
}
...
function ReviewCard({ submission, onDone }: { submission: Submission; onDone: () => Promise<void> }) {
  ...
  return <article className="reviewCard"><header><div><span>{submission.course.code}</span><h3>{submission.title}</h3><p>{submission.student.name} · {submission.student.email}</p></div><a href={submission.repositoryUrl} target="_blank" rel="noreferrer">OPEN SOURCE ↗</a></header><p>{submission.summary}</p><textarea value={feedback} onChange={(event) => setFeedback(event.target.value)} placeholder="Assessment notes, revision requirements, or approval rationale…"/><div className="reviewActions"><span>{error}</span><button disabled={busy} onClick={() => void decide("DECLINED")}>DECLINE</button><button disabled={busy} onClick={() => void decide("REVISION_REQUIRED")}>REQUEST REVISION</button><button disabled={busy} onClick={() => void decide("APPROVED")}>APPROVE + ISSUE CREDENTIAL</button></div></article>;
}

function SubmissionModal({ course, close, save }: { course: Course; close: () => void; save: (payload: Record<string, unknown>) => Promise<void> }) {
  return <div className="modalBack" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><form className="modal" onSubmit={(event) => { event.preventDefault(); void save(Object.fromEntries(new FormData(event.currentTarget))); }}><header><div><p className="kicker">{course.code} / ASSESSED SUBMISSION</p><h2>Submit your mod</h2></div><button type="button" onClick={close}>×</button></header><p>Your studio reviewer will inspect the repository, documentation, and course deliverable before issuing a credential.</p><label>PROJECT TITLE<input name="title" required minLength={3} autoFocus placeholder="Field Logistics Framework"/></label><label>TECHNICAL BRIEF<textarea name="summary" required minLength={30} placeholder="Explain the problem, architecture, testing approach, and what you learned…"/></label><label>REPOSITORY URL<input name="repositoryUrl" required type="url" placeholder="https://github.com/…"/></label><label>DEMO OR WORKSHOP URL <small>OPTIONAL</small><input name="demoUrl" type="url" placeholder="https://…"/></label><div className="modalActions"><button type="button" onClick={close}>CANCEL</button><button className="primary">SEND TO STUDIO REVIEW →</button></div></form></div>;
}
